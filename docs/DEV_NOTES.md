...
| CR-05 | 문법(사용자 결정) | **Enter 확정**: 명령은 완성 즉시 실행하지 않고 Enter 로 실행. 잘못된 Enter 는 실패 표시(버퍼 유지). 긴 방향 토큰의 마침표는 선택(`slash left` = 10자). 피격 원인에 "Enter 누르기 전" 추가. 봇도 Enter 1타 비용 반영(40 WPM 3분 생존 45%→30%) | 사용자 지시 2026-09-17 "타자를 치고 엔터를 눌러야 커맨드가 실행" | 사용자 결정 (DC-07 7-a·commands.json grammar 변경, 기획 세션에 반영 요청) |
...
| IS-09 | 2026-09-20 벡터 파일럿 납품(몸체 5·타일 3·조각·카드 패널 2·아이콘 17 = PNG 38) 임포트 완료(`npm run assets:build`). 남은 placeholder: 오디오 16큐(비프), 기준 그림, 글꼴 파일. 피드백 `docs/FROM_DESIGN.md` | 부분 완료 |
